...
*/
     ...
     double Pr_nf, PrS, Ra_nf ;

int    f ,itr_time, itr_space;
...
FILE *input,*output1,*output2,*output3,*output4,*output5,*output6;
...
f=(k-1)*(k-1);
     ...
     //Ra_nf = Rayleigh_Number( alpha_nf, nu_nf, beta_nf , Th, Tc, gravity, L);
...
for(i=2;i<=k-1;i++)
{
  for(j=2;j<=l-1;j++)
  {
   ...
   else
   {
    ...
    ae2 =-((PrS/pow((1-phi),2.5))*delta_t*f+iue*fabs(u[i][j]*delta_t*(k-1)));
    aw2 =-((PrS/pow((1-phi),2.5))*delta_t*f+iuw*fabs(u[i][j]*delta_t*(k-1)));
    as2 =-((PrS/pow((1-phi),2.5))*delta_t*f+ivs*fabs(v[i][j]*delta_t*(l-1)));
    an2 =-((PrS/pow((1-phi),2.5))*delta_t*f+ivn*fabs(v[i][j]*delta_t*(l-1)));
    ...
    RHS2=(Ra_nf*Pr_nf)*(theta[i+1][j]-theta[i-1][j])*(k-1)/2.0 ;
    zeta[i][j] = (RHS2*delta_t+zeta_old[i][j] - (ae2 * zeta[i+1][j] + aw2*zeta[i-1][j] + as2 * zeta[i][j-1] + an2 * zeta[i][j+1])+(delta_t*PrS/pow((1-phi),2.5)*f)*(STRESS))/ap2;
...
/*calculate nusselt number on hot wall*/
 for(j=1; j<=l; j++)
 {
     Nu_nf[1][j]=(k-1)*(-3.0 * theta[1][j]  + 4.0 * theta[2][j] - theta[3][j]);
 }
 /*print values */
 for(i=1;i<=k;i++)
 {
